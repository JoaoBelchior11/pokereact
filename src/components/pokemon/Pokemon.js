...
export default class Pokemon extends Component {
  ...
  async componentDidMount() {
    ...
    await axios.get(pokemonSpecieUrl).then(resp => {
      ...
      const eggGroups = resp.data["egg_groups"]
        .map(group => {
          return group.name
            .toLowerCase()
            .split("-")
                ...
                <div className="float-right">
                  {this.state.types.map(type => (
                    <span
                      ...
                      style={{ backgroundColor: TYPES[type], color: "white" }}
                    >
                      {" "}
                      {type}
                    </span>
            ...
            <div className="col">
              <div className="col-md-2">
                <img
                  ...
                  className="card-img-top rounded mx-3 mt-4"
                />
              </div>
              <div>
                {this.state.name}
                {statsArray.map(stat => (
                  <PokeStats
                    statName={stat}
                    statValue={this.state.stats[stat]}
                  />
                ))}
              </div>
            </div>
